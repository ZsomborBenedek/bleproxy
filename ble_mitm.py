...
class BLEAdvertisement(dbus.service.Object):
    ...
    def __init__(self, bus, index, device_name, service_uuids):
...
class BLEApplication(dbus.service.Object):
    ...
    def GetManagedObjects(self):
        ...
        log.debug(f"GetManagedObjects called, returning {len(self.services)} service(s): {list(response.keys())}")
        return response
...
async def _enumerate_connected(client) -> EnumResult:
    ...
    for svc in client.services:
        ...
        for char in svc.characteristics:
            uuid = str(char.uuid)
            # Pass all flags through directly — Bleak reads them from BlueZ D-Bus,
...
async def run_proxy(target_mac: str, central_adapter: str, peripheral_adapter: str):
    ...
    async with BleakClient(device, bluez={"adapter": central_adapter}, timeout=30.0, disconnected_callback=on_disconnect, pair=True) as client:
        ...
        enum.device_name = device.name or "BLE Device"
        ...
        gatt_manager.RegisterApplication(
            ...
            error_handler=lambda e: log.error(f"GATT error: {e}")
        )
        adv_manager.RegisterAdvertisement(
            adv.get_path(), {},
            reply_handler=lambda: log.info(f"Advertising as '{enum.device_name}' on {peripheral_adapter} — ready!"),
            error_handler=lambda e: log.error(f"Advertising error: {e}")
        )
